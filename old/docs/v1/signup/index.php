<html>
<head>
    <?php
    $docs_title = "/v1/signup/";

    include_once($_SERVER['DOCUMENT_ROOT'] . "/old/include/docs-functions.php");
    global $docs_end_point;

    $title = "Docs: $docs_title – Notefox";
    include_once($_SERVER['DOCUMENT_ROOT'] . "/old/include/header.php");
    ?>
</head>
<body>
<?php
$selected_menu = "docs";
include_once($_SERVER['DOCUMENT_ROOT'] . "/old/include/menu.php");
?>

<main class="padding-top-menu">
    <div class="horizontal-center ">
        <div class="center-content justify">
            <h1 class="title-section center"><?php echo $docs_title; ?></h1>
            <h2 class="subtitle-section no-bold font-small">
                This API permits to create a new account. Username is just a nickname, email and password are used for
                login.
            </h2>
            <p>
                <b>API Link</b>
                <code>
                    <?php echo $docs_end_point . $docs_title; ?>
                </code>

                <br>
                <b>Method</b>
                <code>POST</code>
                <br>
                <b>Headers</b>
                <code>{ "Content-Type": "application/json" }</code>
            </p>
            <p>
                <b>Request</b>
                <br>
                <code>username</code> (STRING): the username you want to use
                <br>
                <code>email</code> (STRING): the email you want to use
                <br>
                <code>password</code> (STRING): the password you want to use
            </p>
            <p>
                <b>Response (success: code 200)</b>
                <br>
                <code>null</code>
            </p>
            <p>
                <b>Response (error)</b>
                <br>
                <code>400</code>: <?php echo getErrorDescription(400); ?>
                <br>
                <code>401</code>: <?php echo getErrorDescription(401); ?>
                <br>
                <code>416</code>: <?php echo getErrorDescription(416); ?>
                <br>
                <code>419</code>: <?php echo getErrorDescription(419); ?>
            </p>
        </div>
    </div>
</main>

</body>
</html>

<?php
?>