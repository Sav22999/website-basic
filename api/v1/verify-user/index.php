...
$condition = isset($post["email"]) && isset($post["verification-code"]);
if ($condition) {
    $response = null;
    ...
    if ($c = new mysqli($localhost_db, $username_db, $password_db, $database_notefox)) {
        ...
        $verification_code = encryptTextWithPassword($post["verification-code"], $post["email"]);
        ...
        if($result_check->num_rows > 0) {
            if ($row["verified"] === null) {
                if ($row["verification-code"] === $verification_code) {
                    ...
                    $query_update = "UPDATE $users_table SET `status` = 1, `verification-code` = NULL, `verified` = CURRENT_TIMESTAMP WHERE `email` = ? AND `verification-code` = ?";
    ...
    //same code of the commented above one but using also LOCK and UNLOCK of tables to be sure that no one can access the same row
    if ($c = new mysqli($localhost_db, $username_db, $password_db, $database_notefox)) {
        $c->set_charset("utf8mb4");
        ...
        if($result_check->num_rows > 0) {
            if ($row["verified"] === null) {
                if (decryptTextWithPassword($row["verification-code"], $post["email"]) === $verification_code) {
                    //update the status to 1 (verified), verified with the current datetime (getTimestamp()) and verification code to NULL
                    //where email is the email passed and the verification code is the verification code passed
                    $query_update = "UPDATE $users_table SET `status` = 1, `verification-code` = NULL, `verified` = ? WHERE `email` = ?";
                    $stmt_update = $c->prepare($query_update);
                    $stmt_update->bind_param("ss", $now,$email);
